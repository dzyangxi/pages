...
function windowScroll(){
	...
	console.log('documentH:'+(document.documentElement.clientWidth));
	if (checkscrollside()) {

		var oParent = document.getElementById('mainbody');

		for(var i=0; i<getDataSources().data.length; i++){
			//加载
			var OBox = document.createElement('div');
			OBox.className = 'box';
			oParent.appendChild(OBox);
			...
			OImg.src = 'images/books/'+getDataSources().data[i].src;
			OPic.appendChild(OImg);
		}
...
function waterfull (parent,box) {
	...
	oParent.style.cssText = 'width:'+oBoxW*cols+'px; margin:0 aout;';		//设置mainbody宽度和居中
...
function checkscrollside () {
	...
	console.log('lastBoxH:'+lastBoxH+'-->scrollHeight'+scrollHeight+'documentH:'+documentH);
